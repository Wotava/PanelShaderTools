    ...
    def draw(self, context):
        ...
        if len(manager.scene_presets) > 0:
            ...
            if len(preset.layers) > 0:
                ...
                row.prop(current_layer, "plane_offset")

                row = box.row()
                row.prop(current_layer, "plane_dist_A")
                row.prop(current_layer, "plane_dist_B")

                row = box.row()
                row.prop(current_layer, "decal_length")
                row.prop(current_layer, "decal_thickness")
                row.prop(current_layer, "leak_length")

                row = box.row()
                row.label(text="Sectors")
                row.prop(current_layer, "use_FG_mask")
                row.prop(current_layer, "sector_offset")
                row.prop(current_layer, "fg_sectors")
                row.prop(current_layer, "bg_sectors")
